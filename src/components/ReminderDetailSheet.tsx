...
import {
  CalendarClock, Check, FileText, Loader2, Paperclip, Pencil, RotateCcw, Save, Trash2, X,
} from 'lucide-react';
...
const ReminderDetailSheet = ({
  ...
  useEffect(() => {
    ...
    setLocalDueAt(reminder.due_at);
    setLocalStatus(asStatus(reminder.done ? 'done' : reminder.status));
    setEditing(false);
  }, [reminder]);
  ...
  const save = async () => {
    ...
    toast.success('Saved');
    setLocalDueAt(fromLocalInput(due));
    setEditing(false);
  };
  ...
  const postpone = async (days: number) => {
    ...
    const nextDue = shiftDays(localDueAt, days);
    const { error } = await onReschedule(reminder.id, nextDue);
    setChangingStatus(false);
    if (error) return toast.error('Could not reschedule');
    setLocalDueAt(nextDue);
    setDue(toLocalInput(nextDue));
  ...
  const toggleDone = async () => {
    ...
    const { error } = await onToggleDone(reminder.id, done);
    setChangingStatus(false);
    if (error) return toast.error('Could not update this item');
    setLocalStatus(done ? 'done' : 'open');
    toast.success(done ? 'Marked as completed' : 'Reopened');
  };
        ...
        <div className="space-y-5 px-4 pb-10 pt-4">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className={cn('inline-flex items-center gap-1', overdue ? 'font-semibold text-destructive' : 'text-muted-foreground')}>
              <CalendarClock className="h-3.5 w-3.5" />
               {new Date(localDueAt).toLocaleString([], {
          ...
          <div className={cn(
            ...
            <p className="text-sm font-extrabold text-foreground">
              {status === 'done' ? 'Completed' : status === 'postponed' ? `Postponed to ${new Date(localDueAt).toLocaleString([], { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}` : 'Open and needs attention'}
            </p>
          ...
          {!editing ? (
            <div className="space-y-3 rounded-2xl border border-border bg-card p-4">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Notes</p>
                <p className="mt-1 whitespace-pre-wrap text-sm text-foreground">{notes || 'No notes added.'}</p>
              </div>
              <button type="button" onClick={() => setEditing(true)} className="inline-flex items-center gap-1.5 rounded-2xl bg-gradient-primary px-4 py-2.5 text-xs font-bold text-primary-foreground">
          ...
          <div className="space-y-3 rounded-2xl border-2 border-primary/30 bg-primary/5 p-4">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-extrabold text-foreground">Edit reminder details</p>
              <button type="button" onClick={() => setEditing(false)} className="text-xs font-bold text-muted-foreground">Cancel</button>
            </div>
            <div>
